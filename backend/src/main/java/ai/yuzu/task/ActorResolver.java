...
/** v0.0.10 🍊 Resolves ids to verified actors and runs the code-level permission checks of the task system. */
@Component
public class ActorResolver {
    ...
    /** v0.0.10 🍊 Injects the agent and human directories. */
    public ActorResolver(AgentService agents, HumanUserService humans) {
        this.agents = agents;
        this.humans = humans;
    }

    /** v0.0.10 🍊 The actor of an existing human user (NOT_FOUND for unknown ids). */
    ...
    /** v0.0.10 🍊 The actor of an existing agent (NOT_FOUND for unknown ids). */
    public Actor agent(AgentId agentId) {
        return Actor.of(agents.require(agentId));
    }

    /** v0.0.10 🍊 Re-reads the actor, requires a present member of the room, and returns the canonical actor. */
    ...
    /** v0.0.10 🍊 True when the actor may use the permission: humans always, agents only when their scope holds it. */
    ...
    /** v0.0.10 🍊 Throws PERMISSION_DENIED unless the actor may use the permission for the described action. */

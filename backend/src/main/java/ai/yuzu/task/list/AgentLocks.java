...
/** v0.0.20 🍊 One ReentrantLock per agent with a bounded, interruptible wait (never synchronized: virtual threads). */
final class AgentLocks {

    private final Map<AgentId, ReentrantLock> locks = new ConcurrentHashMap<>();
    private final long timeoutMillis;

    /** v0.0.20 🍊 Creates the lock table with the maximum time a caller waits for an agent's lock. */
    AgentLocks(Duration timeout) {
        this.timeoutMillis = timeout.toMillis();
    }

    /** v0.0.20 🍊 Runs the action while holding the agent's lock; CONFLICT on timeout, CANCELLED when interrupted. */

...
/** v0.0.20 🍊 Agent-scoped access to the {@code task_list} table (every statement is bound to :agentId). */
@Repository
public class TaskListRepository extends AgentScopedRepository {
    ...
    /** v0.0.20 🍊 Injects the JDBC client and the JSON helper (goal history column). */
    ...
    /** v0.0.20 🍊 The agent's current list (ACTIVE or AWAITING_APPROVAL), if any. */
    public Optional<TaskListRow> findOpen(AgentId agentId) {
        return scoped(SQL_FIND_OPEN, agentId).query(mapper).optional();
    }

    /** v0.0.20 🍊 One of the agent's lists by id (any status). */
    ...
    /** v0.0.20 🍊 The agent's latest archived lists, most recent first. */
    ...
    /** v0.0.20 🍊 Inserts a new list with a fresh {@code list-<agentHex>-<10hex>} id and returns the id. */
    ...
    /** v0.0.20 🍊 Parses the goal history JSON column (null means no edits yet). */

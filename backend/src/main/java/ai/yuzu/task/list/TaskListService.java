...
/** v0.0.20 🍊 Every agent's ONE current task list: create, apply planning ops, request approval, approve and archive. */
@Service
public class TaskListService implements TicketLinkTarget {

    /** v0.0.20 🍊 Maximum number of operations accepted by one apply call. */
    public static final int MAX_OPS = 50;
    /** v0.0.20 🍊 Archived lists carried by every view ("the last 3 archived task lists and how they went"). */
    ...
    /** v0.0.20 🍊 The agent's current list and latest archived lists (cached per agent, replaced on every change). */
    ...
    /** v0.0.20 🍊 The agent's latest archived lists with their outcomes, most recent first (for planning prompts). */
    ...
    /** v0.0.20 🍊 Creates the agent's current list; CONFLICT while another list is ACTIVE or AWAITING_APPROVAL. */
    ...
    /** v0.0.20 🍊 Applies planning operations to the current list, all or nothing; returns the updated list. */
    public TaskList apply(AgentId agentId, List<TaskOp> ops) {
        return apply(agentId, null, ops);
    }

    /** v0.0.20 🍊 Like {@link #apply(AgentId, List)}, but CONFLICT unless {@code expectedListId} is still current. */
    ...
    /** v0.0.20 🍊 Dry-runs operations on the current list without writing; returns every problem (empty = valid). */
    ...
    /** v0.0.20 🍊 Moves the current list to AWAITING_APPROVAL once every item is DONE or STRUCK (idempotent). */
    ...
    /** v0.0.20 🍊 Archives an AWAITING_APPROVAL list approved by its publisher or any human; clears the current list. */
    ...
    /** v0.0.20 🍊 List side of TicketService.linkList: records the ticket on the open list and moves the ticket. */
    @Override
    ...
    /** v0.0.20 🍊 Runs a write in a transaction, then caches and publishes the fresh view (caller holds the lock). */
    ...
    /** v0.0.20 🍊 The cached view, loaded consistently on a miss (caller holds the agent lock). */
    ...
    /** v0.0.20 🍊 The open list, checked against the expected id; CONFLICT when none, changed, or archived. */
    ...
    /** v0.0.20 🍊 A ticket the agent may start a list for: same room, assigned to it, ASSIGNED/IN_PROGRESS, unlinked. */
    ...
    /** v0.0.20 🍊 Explicit publisher, else whoever assigned the linked ticket (or its creator); BAD_REQUEST if none. */
    ...
    /** v0.0.20 🍊 Moves a linked ticket with its list; failures are reported (UI toast) but never undo the change. */
    ...
    /** v0.0.20 🍊 BAD_REQUEST for an empty or oversized batch. */
    ...
    /** v0.0.20 🍊 "Item 2 is not finished ..." or "Items 2, 4 are not finished ...". */
    ...
    /** v0.0.20 🍊 The agent owning a list id ({@code list-<agentHex>-<10hex>}); NOT_FOUND for anything else. */
    ...
    /** v0.0.20 🍊 NOT_FOUND for an unknown task list. */

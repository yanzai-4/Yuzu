...
/** v0.0.10 🍊 One row of the agent-scoped task_item table (domain form); new items have a null id until inserted. */
public record TaskItemRow(AgentId agentId, String id, String listId, int ord, String text, TaskItemState state,
    ...
    /** v0.0.10 🍊 A new TODO item that is not persisted yet. */
    ...
    /** v0.0.10 🍊 True until the item has been inserted. */
    public boolean isNew() {
        return id == null;
    }

    /** v0.0.10 🍊 Copy in another state. */
    ...
    /** v0.0.10 🍊 Copy with a new note text. */
    ...
    /** v0.0.10 🍊 Copy struck through with a reason (the item stays visible). */
    ...
    /** v0.0.10 🍊 Copy placed in a list (set right before a new item is inserted). */
    ...
    /** v0.0.10 🍊 API view (contract type {@code TaskItem}). */

...
/** v0.0.10 🍊 Persists task-list aggregates (list + items) and loads per-agent views inside the caller's transaction. */
@Component
class TaskListStore {
    ...
    /** v0.0.10 🍊 Injects the repositories and the natural-time renderer used by the views. */
    ...
    /** v0.0.10 🍊 Writes a changed draft: the version-checked list row first, then changed and added items. */
    ...
    /** v0.0.10 🍊 Version-checked list update; CONFLICT when someone else changed the list meanwhile. */
    ...
    /** v0.0.10 🍊 The agent's view: the open list with its items plus the latest archived lists with theirs. */
    ...
    /** v0.0.10 🍊 The latest archived lists with their items, most recent first. */

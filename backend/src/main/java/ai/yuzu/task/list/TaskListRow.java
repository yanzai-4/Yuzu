...
/** v0.0.20 🍊 One row of the agent-scoped task_list table (domain form: publisher, goal history, instants, version). */
public record TaskListRow(AgentId agentId, String id, String goal, List<GoalChange> goalHistory, TaskListStatus status,
    ...
    /** v0.0.20 🍊 Defensive, immutable copy of the goal history. */
    public TaskListRow {
        goalHistory = goalHistory == null ? List.of() : List.copyOf(goalHistory);
    }

    /** v0.0.20 🍊 A new ACTIVE list that is not persisted yet (id assigned on insert). */
    ...
    /** v0.0.20 🍊 Copy after a batch of operations (goal, history, status and completion time may change). */
    ...
    /** v0.0.20 🍊 Copy waiting for the publisher's approval. */
    ...
    /** v0.0.20 🍊 Copy approved and archived with its outcome. */
    ...
    /** v0.0.20 🍊 Copy working on a ticket. */
    ...
    /** v0.0.20 🍊 API view with natural-language times and items in display order (contract type {@code TaskList}). */

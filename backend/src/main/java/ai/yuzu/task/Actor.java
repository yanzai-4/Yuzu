...
/** v0.0.10 🍊 Who acts in the task system: a human user or an AI agent (kind, id, display name). */
public record Actor(Kind kind, String id, String name) {

    private static final int MAX_ID = 16;
    private static final int MAX_NAME = 64;

    /** v0.0.10 🍊 Actor kind, stored in the publisher_kind / creator_kind / assigned_by_kind columns. */
    public enum Kind {
        HUMAN,
        AGENT
    }

    /** v0.0.10 🍊 Validates the shape: agents carry an agent id, humans never do, ids and names fit their columns. */
    public Actor {
        if (kind == null) {
    ...
    /** v0.0.10 🍊 True when both actors are the same person or agent (kind and id; names may differ). */
    public boolean sameAs(Actor other) {
        return other != null && kind == other.kind && id.equals(other.id);
    }

    /** v0.0.10 🍊 Label used in prompts and outcomes, such as "Alice (human)" or "Yuzu (agent)". */

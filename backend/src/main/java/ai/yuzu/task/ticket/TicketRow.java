...
/** v0.0.10 🍊 One row of the room-scoped ticket table (domain form: UTC instants, assigner, optimistic version). */
public record TicketRow(String roomId, String id, String title, String detail, TicketStatus status, Actor creator,
    ...
    /** v0.0.10 🍊 Copy with a new status. */
    ...
    /** v0.0.10 🍊 Copy assigned to an agent by an actor (status ASSIGNED). */
    ...
    /** v0.0.10 🍊 Copy linked to the assignee's task list. */
    ...
    /** v0.0.10 🍊 Copy with the version the database now holds. */
    public TicketRow withVersion(int newVersion) {
        return new TicketRow(roomId, id, title, detail, status, creator, assigneeId, assignedBy, requesterId,
                requesterName, sourceMessageId, listId, newVersion, createdAt, updatedAt);
    }

    /** v0.0.10 🍊 API view with natural-language times (contract type {@code Ticket}). */

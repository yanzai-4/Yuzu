...
/** v0.0.10 🍊 Ticket lifecycle: OPEN → ASSIGNED → IN_PROGRESS → DONE → APPROVED, or CANCELLED. */
public enum TicketStatus {
    ...
    /** v0.0.10 🍊 True for statuses that end the ticket (APPROVED, CANCELLED). */
    public boolean isTerminal() {
        return this == APPROVED || this == CANCELLED;
    }

    /** v0.0.10 🍊 Manual transitions allowed through updateStatus (ASSIGNED is only reached through assign). */
    ...
    /** v0.0.10 🍊 Where a linked ticket moves when its task list reaches {@code target}; null keeps the status. */

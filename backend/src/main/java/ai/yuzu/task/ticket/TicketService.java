...
/** v0.0.10 🍊 Room-level tickets: create, assign, lifecycle moves, task-list links; publishes ticket.upsert. */
@Service
public class TicketService {
    ...
    /** v0.0.10 🍊 Injects collaborators; the link target (task-list service) is resolved lazily to avoid a cycle. */
    ...
    /** v0.0.10 🍊 Creates a ticket (humans or TASK_ASSIGN agents only), optionally assigned at once; publishes it. */
    ...
    /** v0.0.10 🍊 Assigns an OPEN or ASSIGNED ticket to a present agent of its room (humans or TASK_ASSIGN agents). */
    ...
    /** v0.0.10 🍊 Moves a ticket along its lifecycle with per-status permission checks; the same status is a no-op. */
    ...
    /** v0.0.10 🍊 Links the ticket to its assignee's open task list (both sides); ASSIGNED becomes IN_PROGRESS. */
    ...
    /** v0.0.10 🍊 Tickets of a room (the latest 500) in creation order. */
    ...
    /** v0.0.10 🍊 One ticket or NOT_FOUND. */
    public Ticket get(String ticketId) {
        return require(ticketId).toView(time);
    }

    /** v0.0.10 🍊 System transition for the task-list service: links the ticket and follows the list's phase. */
    ...
    /** v0.0.10 🍊 Locks, changes and version-checks one ticket in a transaction; an unchanged row is not written. */
    ...
    /** v0.0.10 🍊 Loads a ticket or throws NOT_FOUND. */
    ...
    /** v0.0.10 🍊 The profile of an agent that can take tickets in the room (present and a member). */
    ...
    /** v0.0.10 🍊 Publishes ticket.upsert once the surrounding transaction commits (immediately when there is none). */
    private void publishAfterCommit(Ticket ticket) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            /** v0.0.10 🍊 Runs the publication after a successful commit only. */
            @Override
    ...
    /** v0.0.10 🍊 True when the list id is a task-list id owned by the agent ({@code list-<agentHex>-<10hex>}). */
    ...
    /** v0.0.10 🍊 NOT_FOUND for an unknown ticket. */

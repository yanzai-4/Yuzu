...
/** v0.0.10 🍊 REST endpoints of the ticket board, the agents' task lists and the human "Approve & archive" button. */
@RestController
@RequestMapping("/api")
public class TaskController {
    ...
    /** v0.0.10 🍊 The room's tickets in creation order (contract type {@code Ticket[]}). */
    @GetMapping("/rooms/{roomId}/tickets")
    public List<Ticket> tickets(@PathVariable String roomId) {
        return tickets.list(roomId);
    }

    /** v0.0.10 🍊 An agent's current task list and latest archived ones (contract type {@code TaskListView}). */
    ...
    /** v0.0.10 🍊 A human approves a finished list, which archives it; returns the agent's new view. */
    ...
    /** v0.0.10 🍊 Body of the approve call: the approving human. */

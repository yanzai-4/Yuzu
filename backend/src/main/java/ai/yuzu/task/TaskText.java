...
/** v0.0.20 🍊 Normalizes and bounds the free text of tickets and task lists (one-line texts keep prompts stable). */
public final class TaskText {
    ...
    /** v0.0.20 🍊 Collapses every whitespace run (newlines included) into one space and strips; null becomes "". */
    public static String oneLine(String raw) {
        return raw == null ? "" : WHITESPACE.matcher(raw).replaceAll(" ").strip();
    }

    /** v0.0.20 🍊 Required one-line text; BAD_REQUEST when blank or longer than {@code max}. */
    ...
    /** v0.0.20 🍊 Optional one-line text; null when blank, BAD_REQUEST when longer than {@code max}. */
    public static String optional(String raw, String field, int max) {
        String text = oneLine(raw);
        return text.isEmpty() ? null : bounded(text, field, max);
    }

    /** v0.0.20 🍊 Multi-line text with normalized line breaks, stripped; "" when null, BAD_REQUEST when too long. */
    public static String multiLine(String raw, String field, int max) {
        String text = raw == null ? "" : raw.replace("\r\n", "\n").replace('\r', '\n').strip();
        return bounded(text, field, max);
    }

    /** v0.0.20 🍊 Enforces the maximum length of a field. */
